...
procedure leer_microprocesadores(var m:microprocesadores);
begin
    write('Escribe la cantidad  de cores: ');
    readln(m.cantidad_cores);
    if (m.cantidad_cores <> 0) then
    begin
        write('Escribe la marca: ');
        readln(m.marca);
        write('Escribe la linea:,');
        readln(m.linea);
        write('Escribe la velocidad');
        readln(m.velocidad);
        write('Escribe los  nanometros: ');
        readln(m.nanometros);
    end;
